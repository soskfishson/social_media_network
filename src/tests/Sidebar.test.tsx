...
describe('Sidebar — data rendering', () => {
    ...
    it('renders both sections even when lists are empty', async () => {
        (global.fetch as jest.Mock)
            .mockResolvedValueOnce({ ok: true, json: async () => [] })
            .mockResolvedValueOnce({ ok: true, json: async () => [] });
    ...
    it('renders multiple users', async () => {
        (global.fetch as jest.Mock)
            .mockResolvedValueOnce({
                ok: true,
                json: async () => [
                    { id: 1, firstName: 'Alice', secondName: 'A', username: 'alice' },
                    { id: 2, firstName: 'Bob', secondName: 'B', username: 'bob' },
                ],
            })
            .mockResolvedValueOnce({ ok: true, json: async () => [] });
...
describe('Sidebar — error handling', () => {
    ...
    it('hides skeletons when API returns non-ok response', async () => {
